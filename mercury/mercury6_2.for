...
c
      ...
      real*8 periq, e, inc, pomega, node, Manom,msum
      character*8 id(NMAX)
...
c------------------------------------------------------------------------------
     ...
     $              /(semi*(1.d0-e**2.d0))
               ...
               !if (mod(time,20.0).eq.0) print *,f,peri,peri+f

               a(1,j) =  -(v(1,j)+3.d0*idotoveri*inc*x(3,j))
     $              /2.d0*adotovera
               a(2,j) =  -(v(2,j)-3.d0*idotoveri*inc*x(3,j))
     $              /2.d0*adotovera
               a(3,j) =  -(v(3,j)+3.d0*idotoveri*inc*x(3,j))
     $              /2.d0*adotovera
               ...
               vu(3,j) = x(3,j)*adotovera
                ...
                if (time.eq.0) then
                    print *, 'time (days):',time
                    print *, 'r:', r
                    print *, 'f:', f
                    print *, 'n:', sqrt(msum)*semi**(-3.d0/2)
                    print *, 'rdot:', rdot
                    print *, 'drdotde:', drdotde
                    print *, 'rfdot:', rfdot
                    print *, 'drfdotde:', drfdotde
                    print *, 'drde:', drde
                    print *, ''
                end if
     ...
     $              *edotovere*e
                    a(3,j) = a(3,j) 
     $              + (drdotde*sin(inc)*sin(peri+f)+drfdotde*sin(inc)
     $              *cos(peri+f)
     $              +drde*cos(inc)*sin(peri+f)*idotoveri*inc)
     $              *edotovere*e
                    vu(1,j) = vu(1,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(1,j)*edotovere
                    vu(2,j) = vu(2,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(2,j)*edotovere
                    vu(3,j) = vu(3,j) + (r/(semi*(1.d0-e**2))-(1.d0
     $                   +e**2)/(1.d0-e**2))*x(3,j)*edotovere
               end if
     ...
     $              idotoveri*(-sin(node)*x(1,j)+cos(node)*x(2,j))
     $              )
     $              *idotoveri*inc
...
c Local
      ...
      real*8 periq, e, inc, pomega, node, Manom
...
c Local
      ...
      real*8 vusr(3,NMAX),vusr0(3,nmax)
c
c------------------------------------------------------------------------------
c
      tol2 = tol * tol
...
c      print *,'in here'
c      print *,opt
      call force (time,jcen,nbod,nbig,mass,x0,v0,s,rcrit,a0
     %  ,stat,ngf,ngflag,opt,nce,ice,jce,vusr)
c      print *,'after'
c
 100  continue
c
...
c
        do k = 2, nbod
          x(1,k) = x0(1,k) + h*(v0(1,k)+vusr(1,k))
          x(2,k) = x0(2,k) + h*(v0(2,k)+vusr(2,k))
          x(3,k) = x0(3,k) + h*(v0(3,k)+vusr(3,k))
          v(1,k) = v0(1,k) + h*a0(1,k)
          v(2,k) = v0(2,k) + h*a0(2,k)
          v(3,k) = v0(3,k) + h*a0(3,k)
        end do
        call force (time,jcen,nbod,nbig,mass,x,v,s,rcrit,a,
     %    stat,ngf,ngflag,opt,nce,ice,jce,vusr)
        do k = 2, nbod
          xend(1,k) = x0(1,k) + hx2*(v(1,k)+vusr(1,k))
          xend(2,k) = x0(2,k) + hx2*(v(2,k)+vusr(2,k))
          xend(3,k) = x0(3,k) + hx2*(v(3,k)+vusr(3,k))
...
c
        do j = 2, n
          call force (time,jcen,nbod,nbig,mass,xend,vend,s,rcrit,a,
     %      stat,ngf,ngflag,opt,nce,ice,jce,vusr)
          do k = 2, nbod
            x(1,k) = x(1,k) + hx2*(vend(1,k)+vusr(1,k))
            x(2,k) = x(2,k) + hx2*(vend(2,k)+vusr(2,k))
            x(3,k) = x(3,k) + hx2*(vend(3,k)+vusr(3,k))
     ...
     %      stat,ngf,ngflag,opt,nce,ice,jce,vusr)
          do k = 2, nbod
            xend(1,k) = xend(1,k) + hx2*(v(1,k)+vusr(1,k))
            xend(2,k) = xend(2,k) + hx2*(v(2,k)+vusr(2,k))
            xend(3,k) = xend(3,k) + hx2*(v(3,k)+vusr(3,k))
...
c
        do k = 2, nbod
          d(1,k,n) = .5d0*(xend(1,k) + x(1,k) 
     $          + h*(vend(1,k)+vusr(1,k)))
          d(2,k,n) = .5d0*(xend(2,k) + x(2,k) 
     $         + h*(vend(2,k)+vusr(2,k)))
          d(3,k,n) = .5d0*(xend(3,k) + x(3,k) 
     $         + h*(vend(3,k)+vusr(3,k)))
...
c Local
      ...
      real*8 ausrh(3,NMAX),vusrh(3,NMAX)
      external mfo_hkce
c
c------------------------------------------------------------------------------
c
...
c Advance interaction Hamiltonian for H/2
      do j = 2, nbod
        v(1,j) = v(1,j)  +  hby2 * (angf(1,j) + a(1,j))
        v(2,j) = v(2,j)  +  hby2 * (angf(2,j) + a(2,j))
        v(3,j) = v(3,j)  +  hby2 * (angf(3,j) + a(3,j))
      end do
c
...
c Advance interaction Hamiltonian for H/2
      call mfo_hy (jcen,nbod,nbig,m,x,rcrit,a,stat)
        if (opt(8).eq.1) then
           call mco_dh2h(time,jcen,nbod,nbig,h0,m,x,v,xh
     $              ,vh,ngf,ngflag,opt)
           call mfo_user (time,jcen,nbod,nbig,m,xh,vh,ausrh,vusrh)
           call mco_h2dh(time,jcen,nbod,nbig,h0,m,vusrh,ausrh,vusr
     $              ,ausr,ngf,ngflag,opt)
        end if
      if (ngflag.eq.1.or.ngflag.eq.3) call mfo_ngf (nbod,x,v,angf,ngf)
c
      do j = 2, nbod
        v(1,j) = v(1,j)  +  hby2 * (angf(1,j) + ausr(1,j)*2 + a(1,j))
        v(2,j) = v(2,j)  +  hby2 * (angf(2,j) + ausr(2,j)*2 + a(2,j))
        v(3,j) = v(3,j)  +  hby2 * (angf(3,j) + ausr(3,j)*2 + a(3,j))
        x(1,j) = x(1,j)  +  hby2 * vusr(1,j)*2
        x(2,j) = x(2,j)  +  hby2 * vusr(2,j)*2
        x(3,j) = x(3,j)  +  hby2 * vusr(3,j)*2
      end do
c
c------------------------------------------------------------------------------
c
...
c Input/Output
      ...
      real*8 a(3,nbod),ngf(4,nbod),rcrit(nbod),vusr(3,NMAX)
c
c Local
      integer j
